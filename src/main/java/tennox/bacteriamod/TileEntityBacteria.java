...
public class TileEntityBacteria extends TileEntity {
    ...
    public TileEntityBacteria() {
        if (food == null) food = new ArrayList<Food>();
        block = Bacteria.bacteria;
        do colony = rand.nextInt(); while (Bacteria.jamcolonies.contains(Integer.valueOf(colony)));
    }
    ...
    public void updateEntity() {
        ...
        if (!startInstantly) {
            if (Bacteria.randomize) tick = rand.nextInt(Bacteria.speed + 1);
    ...
    public static boolean isValidFood(Block block, int meta) {
        if (block == Blocks.bedrock || block == Bacteria.bacteria) return false;
        return true;
    }
    ...
    public boolean isFood(Block block, int meta) {
        if (Bacteria.jamcolonies.contains(Integer.valueOf(colony))) return false;
        ...
        if (block == Blocks.flowing_water || block == Blocks.water) {
            for (Food f : food) {
                if (f.block == Blocks.water || f.block == Blocks.flowing_water) return true;
            }
        }
        if (block == Blocks.flowing_lava || block == Blocks.lava) {
            for (Food f : food) {
                if (f.block == Blocks.lava || f.block == Blocks.flowing_lava) return true;
            }
        }
        return false;
    }
    ...
    private boolean isFood2(Food f, Block block, int meta) {
        if (!block.equals(f.block)) return false;
