...
public class BlockMust extends Block {
    ...
    public int colorMultiplier(IBlockAccess iblockaccess, int i, int j, int k) {
        if (iblockaccess.getBlockMetadata(i, j, k) >= growTime) return 2411556;
        return 16777215;
    }
    ...
    public void updateTick(World world, int i, int j, int k, Random random) {
        if (!world.isRemote) {
            ...
            if (above.equals(Blocks.water)) meta++;
            else if (above.equals(Blocks.flowing_water) && random.nextInt(3) >= 1) meta++;
            if (!above.equals(Blocks.flowing_water) && !above.equals(Blocks.water)) meta = 0;
    ...
    public Item getItemDropped(int i, Random random, int j) {
        if (i >= growTime) return Bacteria.bacteriaBunch;
